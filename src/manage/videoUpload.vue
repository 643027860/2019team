    ...
    <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px">
      ...
      <el-form-item label="组别" prop="videoGroup">
        <el-select v-model="ruleForm.videoGroup" placeholder="选择组别">
          <el-option v-for="(item,index) in groups" :key="index" :label="item.groupName" :value="item.groupName" v-if="groups"/>
        </el-select>
      </el-form-item>
...
<script>
  ...
  import {reqGroups} from '../api/types'
  export default {
    data() {
      return {
        loading : false,
    ...
    methods: {
      ...
      go () {
        ...
        if(videoName.trim()&&videoGroup&&this.editor.$txt.text().trim()&&uploadFile&&updateTime&&createTime){
          this.loading = true
